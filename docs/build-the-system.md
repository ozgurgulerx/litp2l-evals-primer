...
A semantic receipt is valid only for its exact message hash, exact structured-claim hash, case-evidence context hash, criterion, evaluator version, and an accepted calibration-receipt hash. Changing the message, claims, case, request, trace, final state, execution error, or domain policy version invalidates the evidence. The local deterministic runner supplies no free-form semantic evaluator, so an OpenAI-generated customer message remains unqualified until that separate stage is configured. See [the receipt-binding kata](micro-katas.md#kata-02-the-same-message-in-a-different-situation).
