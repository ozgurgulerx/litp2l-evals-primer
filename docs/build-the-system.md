...
A semantic receipt is valid only for its exact message hash, exact structured-claim hash, case-evidence context hash, criterion, evaluator version, and an accepted calibration-receipt hash. Changing the message, claims, case, request, trace, final state, execution error, or domain policy version invalidates the evidence. The runner now accepts an optional evaluator-owned `SemanticStage`, with registry qualification, abstention, and retained judge evidence. It is not configured automatically: an OpenAI-generated customer message remains unqualified until an appropriate stage is supplied. Fixture integration is tested; a live model judge and independently reviewed calibration remain outstanding. See [the receipt-binding kata](micro-katas.md#kata-02-the-same-message-in-a-different-situation) and [Semantic Grading Lab](semantic-grading-lab.md).
...
The current slice does not yet claim multi-turn simulation, live retrieval evaluation, a calibrated LLM judge, production-grade statistical inference, production traffic, or complete live cost accounting. The [CI Gate Lab](ci-gate-lab.md) now provides locally executed conformance checks and a configured GitHub Actions dependency for book publication; cloud execution and application deployment enforcement remain unverified. Typed scorer fixtures and a teaching interval are not architecture transfer evidence. Each later capability needs its own trace-producing runner, tests, evidence, and failure examples.
