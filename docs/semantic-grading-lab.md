...
??? success "Solution: compute the known estimate and preserve missingness"
    ...
    The paired runner's `cost_usd` represents agent cost. Judge cost and elapsed time remain separate in semantic audit. [Katas 38–40](eval-operations-integrity.md#kata-38-is-an-unknown-bill-free-budget) now add selected-cost aggregation, durable estimated-cost reservations and judge admission stops. Complete operational accounting, agent/tool budgets and provider cancellation remain open. Do not describe the packet's agent-cost column as total spend.
