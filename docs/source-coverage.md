...
These are depth gaps in already-covered subjects, not reasons to discard the existing chapters. Paired slice reporting, mixed-trace error analysis, evidence packing and fixed-design sampling now have local worked artifacts. Next depth work should address quantitative rater effects, judge-design robustness and complete report/citation evidence; keep external framework runs, actual reviewer studies and model-backed transfer scoped to their prerequisites. No single study closes the entire curriculum.
