...
These are depth gaps in already-covered subjects, not reasons to discard the existing chapters. Paired slice reporting, mixed-trace error analysis, evidence packing, fixed-design sampling and synthetic rater analysis now have local worked artifacts. Next depth work should address judge-design robustness and complete report/citation evidence; keep external framework runs, actual reviewer studies and model-backed transfer scoped to their prerequisites. No single study closes the entire curriculum.
