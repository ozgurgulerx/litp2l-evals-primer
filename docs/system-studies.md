...
The [Coding Patch Evaluation Study](coding-patch-study.md) adds actual execution of authored patches against visible and separate acceptance checks. Katas 87–89 connect outcome correctness, caller-state preservation, protected-test integrity and test validity. Eight child processes produce forty case checks per study; the model that might generate a patch is not part of this experiment.
