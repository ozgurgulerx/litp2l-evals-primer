...
**Executed narrower control:** the [Cross-Run Isolation Study](cross-run-isolation-study.md) now runs actual SQLite cache operations across fresh, reused and concurrently active workers. Katas 56–57 contrast private leakage with allowed same-run and shared-policy access. This closes the application-cache example only. The sandbox, separate logging authority and other communication channels in the broader protocol above remain unverified.
